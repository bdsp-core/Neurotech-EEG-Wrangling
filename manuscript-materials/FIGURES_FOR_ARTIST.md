...
## ✅ RESOLVED (2026-08) — Figure 1 and Supp Fig 2 are now generated in code (no artist needed)
`figure1_pipeline.png` is regenerated by `make_figure1_pipeline.py` and `supp_figure2_deidentification.png`
by `make_supp_figure2_deid.py`, both with the corrected facts (Lifelines/EMS + Persyst; 23,607 EDF
recording segments; equipment **removed**, not kept). They are now reproducible and code-controlled. If a
nicer hand-drawn version is preferred, drop the PNG in and re-run the docx generators. (Original artist spec
retained below for reference.)

## (superseded) Corrections after Keith Morgan's review — the two hand-made figures had been factually WRONG
