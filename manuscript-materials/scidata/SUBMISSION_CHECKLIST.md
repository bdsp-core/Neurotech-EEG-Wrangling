...
6. **Published sidecars: DONE (2026-09-11).** All 54,346 `*_eeg.json` sidecars now say `Lifelines/EMS`; verified by 300 random re-reads. The 10 sidecars that were missing outright were regenerated and uploaded.
6b. **Published tree is incomplete (full S3 listing, 2026-09-10).** 107 sessions listed in the recordings table were never uploaded (7 signal-bearing, 53 h; bucket versioning confirms no deletion), 31 sessions have sidecars but no EDF (the 8 sessions that lacked a sidecar are fixed). The 107 absent + 31 EDF-less sessions need the Padlock_DT source drive; both linking tables are recovered and `restore_missing_sessions.py` is ready (Fix 3). Restore before reviewers are invited; the journal checks the repository from the second review round. The manuscript numbers describe the complete built release.
