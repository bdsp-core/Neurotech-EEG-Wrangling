...
6. **Published sidecars.** `patch_s3_manufacturer.py` rewrites `Manufacturer` in all 54,346 `*_eeg.json` sidecars from `Natus/Xltek` to `Lifelines/EMS`. Running it from Claude Code was blocked by the auto-mode permission classifier (cloud mass-modification), so it has NOT been run; run it yourself (dry-run first, see the script docstring).
6b. **Published tree is incomplete (found by a full S3 listing on 2026-09-10).** 107 sessions listed in the recordings table are absent from S3 (7 signal-bearing, 53 h), 31 sessions have sidecars but no EDF, and 9 sessions lack a sidecar. Details, file lists, and restore commands: `scidata/s3_audit_2026-09-10/`. The source BIDS files are not on this machine. Restore before reviewers are invited (the journal checks the repository from the second review round); the manuscript numbers describe the complete built release.
